...
> **Примечание:** подробнее о порядке работы событий рисования см. в разделе [**«События отрисовки»**](https://manual.gamemaker.io/beta/en/The_Asset_Editors/Object_Properties/Draw_Events.htm).

## Цели отрисовки

**GameMaker** всегда рисует на **цель отрисовки**, автоматически меняя её в определённые моменты цикла рисования и затем сбрасывая. В качестве цели отрисовки может выступать:

* буфер дисплея:
* * в событиях *«Перед отрисовкой»* и *«После отрисовки»* (см. [**«События отрисовки»**](https://manual.gamemaker.io/beta/en/The_Asset_Editors/Object_Properties/Draw_Events.htm);
...
По умолчанию GameMaker использует поверхность приложения для отрисовки в стандартных событиях отрисовки. Однако можно отключить её автоматическое рисование с помощью [**`application_surface_draw_enable`**](https://manual.gamemaker.io/beta/en/GameMaker_Language/GML_Reference/Drawing/Surfaces/application_surface_draw_enable.htm) или вовсе выключить её через [**`application_surface_enable`**](https://manual.gamemaker.io/beta/en/GameMaker_Language/GML_Reference/Drawing/Surfaces/application_surface_enable.htm). Если поверхность приложения отключена, всё рисуется напрямую в буфер дисплея.
