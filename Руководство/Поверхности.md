...
* —> по умолчанию поверхность приложения отрисовывается в *буфер дисплея* (вы можете отключить это с помощью функции [**``application_surface_draw_enable()``**](https://manual.gamemaker.io/beta/en/GameMaker_Language/GML_Reference/Drawing/Surfaces/application_surface_draw_enable.htm));
